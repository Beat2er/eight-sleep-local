...
async def async_setup_entry(
    ...
    entities = [
        ...
        EightSleepAlarmVibratingSensor(coordinator, entry.entry_id, "left"),
        EightSleepAlarmVibratingSensor(coordinator, entry.entry_id, "right"),
    ]
...
class EightSleepPresenceSensor(CoordinatorEntity, BinarySensorEntity):
    ...
    def extra_state_attributes(self) -> dict:
        ...
        presence_data = data.get("_presence", {})
        side_presence = presence_data.get(self._side, {})
        # Support both field names (lastUpdatedAt from upstream API)
        last_updated = side_presence.get("lastUpdatedAt") or side_presence.get("lastUpdated")
        return {
...
class EightSleepAlarmVibratingSensor(CoordinatorEntity, BinarySensorEntity):
    """Binary sensor for alarm vibrating state."""

    _attr_device_class = BinarySensorDeviceClass.VIBRATION
    _attr_icon = "mdi:vibrate"

    def __init__(self, coordinator, entry_id: str, side: str) -> None:
        """Initialize the alarm vibrating sensor."""
        super().__init__(coordinator)
        self._side = side
        self._entry_id = entry_id
        self._attr_name = f"Eight Sleep {side.capitalize()} Alarm Vibrating"
        self._attr_unique_id = f"eight_sleep_{side}_alarm_vibrating"

    @property
    def is_on(self) -> bool:
        """Return true if alarm is currently vibrating."""
        data = self.coordinator.data or {}
        side_data = data.get(self._side, {})
        return side_data.get("isAlarmVibrating", False)
